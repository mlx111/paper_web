...
class ResearchRegenerationTest(unittest.TestCase):
    ...
    def test_reload_and_regenerate_report_from_existing_artifacts(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            artifacts = service.reload_research_artifacts("research_session_abc")
            regenerated = service.regenerate_report("research_session_abc")

            self.assertEqual(artifacts["manifest"]["report_version"], "v2.3")
            self.assertTrue(Path(regenerated["manifest_path"]).exists())
            self.assertIn("## 核心发现", Path(regenerated["report_path"]).read_text(encoding="utf-8"))
