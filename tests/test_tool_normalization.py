...
class TestToolWrapper:
    ...
    def test_execute_with_trace_records_successful_tool_step(self, tmp_path):
        from langchain_core.tools import tool as langchain_tool
        from app.services.run_trace_service import RunTraceService

        @langchain_tool(name_or_callable="echo", description="echoes input")
        def echo(text: str) -> str:
            return ToolResult.success(data={"text": text}, summary="echoed text").to_message_content()

        reg = ToolRegistry()
        reg.register(ToolMeta(name="echo", description="d", category=ToolCategory.UTILITY, tool_ref=echo))
        wrapper = ToolWrapper(reg)
        trace_service = RunTraceService(base_dir=tmp_path)
        run = trace_service.start_run(session_id="s1", route="test", question="q")

        result = wrapper.execute_with_trace(
            "echo",
            {"text": "hello", "api_key": "secret-value"},
            trace_service=trace_service,
            run_id=run.run_id,
            node_name="tools",
        )

        trace = trace_service.load_run(run.run_id)
        step = trace["steps"][0]
        assert result.ok is True
        assert step["step_name"] == "tool:echo"
        assert step["step_type"] == "tool"
        assert step["status"] == "completed"
        assert step["input"]["arguments"]["api_key"] == "***"
        assert step["output"]["ok"] is True
        assert step["output"]["summary"] == "echoed text"
        assert step["output"]["data_size"] > 0

    def test_execute_with_trace_records_failed_tool_step(self, tmp_path):
        from langchain_core.tools import tool as langchain_tool
        from app.services.run_trace_service import RunTraceService

        @langchain_tool(name_or_callable="boom", description="fails")
        def boom(text: str) -> str:
            raise RuntimeError("tool exploded")

        reg = ToolRegistry()
        reg.register(ToolMeta(name="boom", description="d", category=ToolCategory.UTILITY, tool_ref=boom))
        wrapper = ToolWrapper(reg)
        trace_service = RunTraceService(base_dir=tmp_path)
        run = trace_service.start_run(session_id="s1", route="test", question="q")
        ...
        assert step["output"]["ok"] is False
