...
class TestCheckpointManager:
    ...
    def test_get_progress_failed_checkpoint_is_resumable(self, tmp_path):
        store = CheckpointStore(base_dir=tmp_path / "checkpoints")
        store.save(Checkpoint(
            session_id="s1",
            workflow_name="wf",
        ...
        assert progress["latest_step"] == 1
        assert progress["latest_step_name"] == "synthesize"
        assert progress["error"] == "model timeout"
