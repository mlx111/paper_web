...
class MemorySelector:
    ...
    def _tokenize(text: str) -> set[str]:
        ...
        Uses ``\\w+`` for ASCII tokens and jieba segmentation for CJK
        sub-tokenization so Chinese queries don't get treated as single
        opaque strings.
        """
        lowered = text.lower()
        terms = set(re.findall(r"\w+", lowered))

        # If text contains CJK, add jieba-segmented terms for finer matching
        if MemorySelector._CJK_RE.search(lowered):
            jieba_terms = jieba.cut(lowered)
            terms.update(t.strip() for t in jieba_terms if t.strip())
